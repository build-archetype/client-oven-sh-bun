  ...
  if (os === "darwin") {
    ...
    return {
      ...
      command: [
        ...
        `  if [ -n "$VM_IP" ] && sshpass -p admin ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null admin@$VM_IP echo "VM is healthy" > /dev/null 2>&1; then`,
        '    echo "VM is healthy"',
        '    break',
        '  fi',
        ...
        `sshpass -p admin ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null admin@$VM_IP 'cd ~/workspace/workspace && ${getBuildCommand(platform, options)} --target dependencies 2>&1 | tee /tmp/build.log'`,
