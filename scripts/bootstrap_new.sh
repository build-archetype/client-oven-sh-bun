#!/bin/sh
# Version: 14 - Fixed Rust toolchain setup: properly install default stable toolchain

# A script that installs the dependencies needed to build and test Bun.
# This should work on macOS and Linux with a POSIX shell.
# Enhanced version with better macOS support while maintaining full Linux compatibility.
...
append_to_profile() {
	...
	for profile in $profiles; do
		for profile_path in "$current_home/$profile" "$home/$profile"; do
			# Create profile file if it doesn't exist (especially important for CI/VM environments)
			if [ "$ci" = "1" ] || [ -f "$profile_path" ]; then
				append_file "$profile_path" "$content"
			elif [ "$profile" = ".zprofile" ] || [ "$profile" = ".profile" ]; then
				# Always create essential profiles on macOS VMs
				append_file "$profile_path" "$content"
			fi
		done
	done
...
install_xcode_tools() {
	...
	if command -v clang >/dev/null 2>&1 && command -v clang++ >/dev/null 2>&1; then
		...
		if xcode-select -p >/dev/null 2>&1; then
			...
			if echo "$xcode_path" | grep -q "/Applications/Xcode"; then
...
install_llvm() {
	...
	brew)
		...
		# Enhanced: Add symlinks for easier access on macOS
		if [ "$os" = "darwin" ]; then
			...
			execute_sudo ln -sf "$llvm_prefix/bin/clang" "$bin_dir/clang"
			execute_sudo ln -sf "$llvm_prefix/bin/clang++" "$bin_dir/clang++"
			execute_sudo ln -sf "$llvm_prefix/bin/llvm-ar" "$bin_dir/llvm-ar"
			execute_sudo ln -sf "$llvm_prefix/bin/llvm-ranlib" "$bin_dir/llvm-ranlib"
			execute_sudo ln -sf "$llvm_prefix/bin/lld" "$bin_dir/lld"
			execute_sudo ln -sf "$llvm_prefix/bin/llvm-symbolizer" "$bin_dir/llvm-symbolizer"
...
install_rust() {
	...
	*)
		...
		# CRITICAL FIX: Install and set default Rust toolchain
		print "Setting up default Rust toolchain..."
		rustup="$rust_home/bin/rustup"
		if [ -f "$rustup" ]; then
			# Set up environment for this installation session
			export RUSTUP_HOME="$rust_home"
			export CARGO_HOME="$rust_home"
			export PATH="$rust_home/bin:$PATH"
			
			# Install and set default stable toolchain
			execute "$rustup" default stable
			print "✅ Rust default toolchain configured"
			
			# Verify installation
			cargo_bin="$rust_home/bin/cargo"
			rustc_bin="$rust_home/bin/rustc"
			if [ -f "$cargo_bin" ] && [ -f "$rustc_bin" ]; then
				print "✅ Rust installation verified: cargo and rustc available"
			else
				print "⚠️ Rust installation may have issues - tools not found"
			fi
			
			# Create a dedicated Rust environment file for reliable sourcing
			rust_env_file="$rust_home/env"
			cat > "$rust_env_file" << EOF
# Rust environment setup - Auto-generated by bootstrap script
export RUSTUP_HOME="$rust_home"
export CARGO_HOME="$rust_home"
export PATH="$rust_home/bin:\$PATH"
EOF
			
			# Add sourcing of Rust environment to shell profiles
			append_to_profile "# Source Rust environment"
			append_to_profile "[ -f \"$rust_env_file\" ] && source \"$rust_env_file\""
			
			print "✅ Rust environment configuration completed"
		else
			print "❌ rustup not found after installation"
		fi
		;;
	esac
...
install_chromium() {
	...
	dnf | yum)
		...
		;;
	brew)
		# Enhanced: Add macOS support for browser testing
		print "Installing browser for testing..."
		execute_as_user brew install google-chrome --cask || {
			...
			execute_as_user brew install chromium --cask || {
				print "⚠️ Browser installation failed - browser tests may not work"
				return 1
			}
		}
