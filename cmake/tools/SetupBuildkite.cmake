...
if(UPLOAD_ZIG_CACHES AND BUILDKITE_CACHE AND BUILDKITE AND (CACHE_STRATEGY STREQUAL "read-write" OR CACHE_STRATEGY STREQUAL "write-only") AND IS_DIRECTORY ${CACHE_PATH}/zig/local)
  ...
  register_command(
    ...
    COMMAND /bin/sh -c "
      echo 'Checking Zig local cache content before upload...'
...
if(UPLOAD_ZIG_CACHES AND BUILDKITE_CACHE AND BUILDKITE AND (CACHE_STRATEGY STREQUAL "read-write" OR CACHE_STRATEGY STREQUAL "write-only") AND IS_DIRECTORY ${CACHE_PATH}/zig/global)
  ...
  register_command(
    ...
    COMMAND /bin/sh -c "
      echo 'Checking Zig global cache content before upload...'
...
# This must happen before cache restoration to ensure link step always gets artifacts
...
if(BUN_LINK_ONLY)
  ...
  message(STATUS "Build artifact downloading setup complete")
else()
  message(STATUS "Not a link step - skipping build artifact downloading")
endif()
    ...
    if(download_result EQUAL 0 AND EXISTS ${BUILD_PATH}/${cache_artifact})
      ...
      if(extract_result EQUAL 0)
        ...
        if(file_count GREATER 0)
          ...
          message(STATUS "  ERROR: ${cache_artifact} is empty or corrupted (${file_count} files) - treating as cache miss")
          # Clean up the empty cache directory
          file(REMOVE_RECURSE ${cache_dir})
          file(MAKE_DIRECTORY ${cache_dir})
        endif()
      else()
        message(STATUS "  WARNING: Failed to extract ${cache_artifact}")
      endif()
      
      # Clean up downloaded archive
      file(REMOVE ${BUILD_PATH}/${cache_artifact})
    else()
      message(STATUS "  INFO: No ${cache_artifact} found (exit code: ${download_result})")
      if(download_output)
